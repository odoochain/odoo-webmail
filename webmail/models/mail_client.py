...
from odoo.addons.webmail.models import mail_utils
        ...
        try:
            ...
            if self.folder_exists(dest_path):
                ...
                if ret == 'create completed':
                    ...
                    raise mail_utils.CreateFolderFailed(f"创建邮箱文件夹 {dest_path} 失败")
    ...
    def handle_mails(self, func, folder_name='Inbox', criteria='UNSEEN', mails_count=50,
        ...
        if messages:
            ...
            for uid, message_data in tqdm(self.fetch(messages, 'RFC822').items()):
                ...
                try:
                    mail_info = mail_utils.eml_to_mail_info(eml, attach_filter=attach_filter, html_parser=html_parser)
                except mail_utils.DecodeException as e:
